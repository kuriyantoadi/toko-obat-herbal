...
if (!empty($_GET['search'])) {
    ...
    $where[] = "(p.nama_produk LIKE '%$search%' OR p.deskripsi_produk LIKE '%$search%')";
}

// Handle filter kategori
if (!empty($_GET['kategori'])) {
    $kategori_filter = (int)$_GET['kategori'];
    $where[] = "p.id_kat_produk = $kategori_filter";
}
...
// Hitung total produk untuk "Semua Kategori"
$result_all = mysqli_query($koneksi, "
    SELECT COUNT(*) as total 
    FROM tb_produk p 
    JOIN tb_kategori_produk k ON p.id_kat_produk = k.id_kat_produk
    " . (!empty($search) ? "WHERE (p.nama_produk LIKE '%$search%' OR p.deskripsi_produk LIKE '%$search%')" : "")
);
$total_all_produk = mysqli_fetch_assoc($result_all)['total'];
                        ...
                        <div class="card-body">
                            <ul class="list-group mb-3">
                                ...
                                <li class="list-group-item border-0 p-0">
                                    <a class="<?= $kategori_filter == '' ? 'fw-bold text-primary' : '' ?>" 
                                       href="produk.php?search=<?= urlencode($search) ?>">
                                        <i class="fe fe-grid"></i> Semua Kategori
                                    </a>
                                    <span class="product-label">(<?= $total_all_produk ?>)</span>
                                </li>
                            </ul>
                            ...
                            <ul class="list-group">
                                ...
                                while ($kat = mysqli_fetch_array($kategori)) {
                                    $active_class = ($kategori_filter == $kat['id_kat_produk']) ? 'fw-bold text-primary' : '';
                                ?>
                                    <li class="list-group-item border-0 p-0">
                                        <a class="<?= $active_class ?>" 
                                           href="produk.php?kategori=<?= $kat['id_kat_produk'] ?>&search=<?= urlencode($search) ?>">
                        ...
                        <div class="float-end">
                            <ul class="pagination">
                                <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                                    <a class="page-link" href="?page=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&kategori=<?= urlencode($kategori_filter) ?>">&laquo;</a>
                                </li>
                                <?php for ($i = 1; $i <= $total_page; $i++) { ?>
                                    <li class="page-item <?= $page == $i ? 'active' : '' ?>">
                                        <a class="page-link" href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&kategori=<?= urlencode($kategori_filter) ?>"><?= $i ?></a>
                                    </li>
                                <?php } ?>
                                <li class="page-item <?= $page >= $total_page ? 'disabled' : '' ?>">
                                    <a class="page-link" href="?page=<?= $page + 1 ?>&search=<?= urlencode($search) ?>&kategori=<?= urlencode($kategori_filter) ?>">&raquo;</a>
                                </li>
                            </ul>
                        </div>
                    </div>
